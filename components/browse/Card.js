...
export default function Card({ item, section, mouseDown, dragging, isMobile }) {
  ...
  const onPosterClick = (e, item) => {
    ...
    console.log("onPosterClick", item)
    setSelectedItem({
      id: item.id,
    ...
    <styled.CardContainer key={item.id} dragging={dragging} isMobile={isMobile}>
      ...
      >
        ...
        {(showPoster(item) || !sliderTrailer?.isLoaded) && (
          <styled.Poster>
            {/* https://images.weserv.nl/?url=${item.backdrop_path}&w=300&h=300 */}
            <Image
              src={`${section?.size === 'large'
                ? !item.poster_path.includes("https") ? `https://image.tmdb.org/t/p/w342/${item.poster_path}` : `${item.poster_path}`
                : !item.backdrop_path.includes("https") ? `https://image.tmdb.org/t/p/w300/${item.backdrop_path}` : `${item.backdrop_path}`
                }`}
